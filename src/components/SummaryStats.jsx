    ...
    <div className="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3 text-sm">
      ...
      >
        ...
        {(() => {
          let colorClass = "";
          if (average >= 100)
            colorClass = isDark ? "text-green-300" : "text-green-700";
          else if (average >= 75)
            colorClass = isDark ? "text-yellow-300" : "text-yellow-700";
          else colorClass = isDark ? "text-red-300" : "text-red-700";
          return <span className={`font-bold ${colorClass}`}>{average}</span>;
        })()}
      </div>
      <div
        className={`${
      ...
      >
        ...
        {(() => {
          ...
          return <span className={`font-bold ${colorClass}`}>{total}</span>;
        })()}
      </div>
    </div>
  );
